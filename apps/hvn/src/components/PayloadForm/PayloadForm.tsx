    ...
    <section className="min-h-screen py-12 bg-gradient-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4 max-w-3xl">
        <Card className='border-2 shadow-lg'>
          <CardHeader>
            <CardTitle>{getLabelFromForm('title', formData.title)}</CardTitle>
              ...
              {formData.fields?.map((field, index) => {
                ...
                return (
                  <div className="space-y-2" key={index}>
                    {(label && blockType !== 'checkbox') && <Label htmlFor={name}>{label} {required && <span className="text-red-500">*</span>}</Label>}

                    {blockType === 'text' && <Input id={name} {...register(name)} required={required} />}

                    {blockType === 'textarea' && (
                      <Textarea id={name} {...register(name)} rows={5} />
                    )}
                    ...
                    {blockType === 'number' && <Input id={name} type="number" {...register(name)} required={required} />}
